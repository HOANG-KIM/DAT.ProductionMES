...
- **Format mục "Lịch sử triển khai" (áp dụng từ 24/08/2026 trở đi, không hồi tố entry cũ):** mỗi đợt cập nhật là 1 heading `#### YYYY-MM-DD (ai, mô tả ngắn)` riêng, bên dưới là bullet xuống dòng theo từng phần đã đổi — KHÔNG viết dồn thành 1 đoạn văn liền mạch như các entry trước 24/08/2026 (khó đọc). Chỉ liệt kê bullet cho phần thực sự có đổi, ví dụ:
  ```
  #### 2026-08-24 (dev, mô tả ngắn)
  - **Domain:** ...
  - **Application:** ...
  - **Infrastructure:** ...
  - **Api:** ...
  - **web-admin:** ...
  - **Station.Wpf:** ...
  - **Test:** ... (số lượng test, kết quả pass)
  - **Build:** `dotnet build`/`npm run build` kết quả
  - **Còn thiếu / lý do giữ 🟡 (nếu có):** ...
  ```
