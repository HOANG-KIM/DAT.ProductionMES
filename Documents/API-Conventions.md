...
- **Ngoại lệ đã chốt 19/08/2026 (yêu cầu trực tiếp người dùng vận hành)**: 4 field `Scan.ScannedAtUtc`, `ReworkUnlock.UnlockedAtUtc`, `Lot.UpdatedAtUtc`, `LotHistory.ChangedAtUtc` (bổ sung 20/08/2026, cùng lý do — ghi kèm mỗi lần `Lot.UpdatedAtUtc` đổi, phải cùng mốc giờ để đối chiếu được) (và mọi DTO phái sinh từ chúng, vd `ScanHistoryItemDto.ScannedAtUtc`, `LotSummaryDto.FirstScannedAtUtc`, `ReworkLockStatusDto.NgScannedAtUtc`, `LotHistoryItemDto.ChangedAtUtc`) **KHÔNG còn theo quy ước UTC ở trên** — lưu và trả về đúng **giờ tường tại nhà máy (giờ Việt Nam, UTC+7)**, KHÔNG quy đổi ở bất kỳ tầng nào (ghi bằng `DateTime.Now` phía server thay vì `DateTime.UtcNow`, JSON trả về KHÔNG có hậu tố `Z`). Lý do: người vận hành cần "scan lúc mấy giờ thì lưu/hiển thị đúng đúng giờ đó", chấp nhận đánh đổi rủi ro nếu sau này server đổi timezone (hiện tại server luôn chạy cùng múi giờ nhà máy, hệ thống 1-site). Giữ nguyên tên field có hậu tố `Utc` (không đổi tên, tránh vỡ hợp đồng API/DTO ở cả `web-admin` lẫn `Station.Wpf`) nhưng đây là NGOẠI LỆ đã biết — đọc XML doc tại chính field đó trước khi dùng để tránh nhầm lẫn. Field xác thực/token (`RefreshToken.ExpiresAtUtc`/`CreatedAtUtc`/`RevokedAtUtc`, `StationApiKey.CreatedAtUtc`/`RevokedAtUtc`) **VẪN giữ UTC thật** theo quy ước gốc — KHÔNG áp dụng ngoại lệ này, vì JWT `exp` claim nội tại do thư viện tự tính theo UTC (RFC 7519), đổi field DB liên quan có thể gây lệch logic hết hạn phiên đăng nhập/API Key.
