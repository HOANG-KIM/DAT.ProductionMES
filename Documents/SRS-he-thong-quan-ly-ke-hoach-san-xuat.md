...
- Khi scan thành công (kết quả OK), hiển thị **thông báo xác nhận đã lưu** (dạng popup/toast) cho công nhân biết lượt scan đã được ghi nhận, kèm mã tem vừa scan.
  ...
  - Khi scan lỗi (Trùng tem / Chưa qua công đoạn trước / đang khóa rework / NG chủ động...), thông báo phải có màu sắc và âm thanh khác biệt rõ ràng với thông báo OK.
  - **Sửa lại 25/08/2026 (xem FR-27)**: với lỗi do hệ thống **tự động từ chối** (Trùng tem/Chưa qua công đoạn trước/Đang khóa rework/...), thông báo không còn chỉ "xác nhận đã đọc" đơn thuần — có 2 lựa chọn **"Lưu"** (yêu cầu Tổ trưởng đăng nhập xác nhận mới lưu vào lịch sử) và **"Thoát"**/Esc (đóng ngay, không lưu gì). Riêng lượt Scan NG **chủ động** (người vận hành tự quét ở Chế độ Scan NG, US-18) không thuộc nhóm này — vẫn lưu ngay sau khi xác nhận, không đổi.
...
- Lượt scan OK được lưu lại ngay lập tức: mã tem, thời gian, trạm, công đoạn, Line, kế hoạch, kết quả, người thao tác.
- **Sửa lại 25/08/2026 (xem FR-27)**: lượt scan bị **hệ thống tự động từ chối** (Trùng tem/Chưa qua công đoạn trước/Đang khóa rework/...) **không còn tự động lưu ngay** — chỉ được lưu khi Tổ trưởng/Admin/Manager đăng nhập xác nhận "Lưu" (FR-27). Lượt Scan NG chủ động (US-18) không thuộc thay đổi này, vẫn lưu ngay sau khi xác nhận như cũ.
...
- Tem trùng tại công đoạn này bị từ chối **đúng theo quy tắc mặc định của FR-08 — không có ngoại lệ ghi đè**. **Sửa lại 25/08/2026 (xem FR-27, mục 6 quy tắc 16 mới)**: việc lưu lịch sử lượt trùng tem này nay dùng chung cơ chế Lưu/Thoát của FR-27 (không còn cơ chế audit riêng "luôn lưu + xác nhận-đã-biết tùy chọn" như bản gốc 20/08/2026) — Supervisor phải đăng nhập bấm "Lưu" thì bản ghi mới được tạo, bấm "Thoát" thì không có bản ghi nào (kể cả audit). Không áp dụng cho bất kỳ công đoạn nào khác ngoài phạm vi chung của FR-27.
- Lượt scan OK tại công đoạn này luôn lưu lịch sử ngay (không đổi). Các lượt bị từ chối (chưa qua công đoạn trước, trùng tem) áp dụng đúng cơ chế Lưu/Thoát chung của FR-27 — không còn tự động lưu.
...
**FR-27 — Xác nhận Tổ trưởng trước khi lưu lịch sử scan bị từ chối** *(bổ sung 25/08/2026 — đảo ngược 1 phần FR-10/mục 6 quy tắc 6)*
- Lượt scan bị **hệ thống tự động từ chối** (Trùng tem/FR-08, Chưa qua công đoạn liền trước/FR-08, Đang khóa rework/FR-19, và mọi kết quả từ chối tự động phát sinh sau này — kể cả tại công đoạn "Đóng thùng", xem FR-25) **không còn tự động lưu lịch sử ngay lúc scan**. Hệ thống hiển thị thông báo tại trạm kèm 2 lựa chọn **"Lưu"** và **"Thoát"** (hoặc Esc = Thoát).
- Bấm **"Thoát"**: đóng thông báo ngay, **không lưu bất kỳ bản ghi nào** (kể cả bản ghi audit) — nếu quét lại đúng tem đó, hệ thống kiểm tra lại từ đầu như chưa từng có lượt scan nào trước đó.
- Bấm **"Lưu"**: yêu cầu Tổ trưởng/Admin/Manager đăng nhập xác nhận (re-auth mỗi lần, cùng cơ chế đã dùng ở US-18/US-19). Đăng nhập thành công → lưu 1 bản ghi lịch sử scan đúng theo kết quả/lý do đã hiển thị, kèm danh tính người xác nhận, thời gian **gốc lúc scan** (không phải lúc xác nhận) → thông báo "Lưu thành công" tự động biến mất sau 1-2 giây. Đăng nhập sai/thiếu quyền/Hủy → không lưu gì, có thể thử lại hoặc quay về thông báo gốc.
- **Ngoại lệ — không áp dụng cho lượt Scan NG chủ động (US-18, FR-18)**: người vận hành tự quét sản phẩm lỗi ở Chế độ Scan NG vẫn lưu ngay sau khi xác nhận NG như hiện có, không đổi gì (khác bản chất — đây là hành động chủ động, không phải hệ thống tự động từ chối).
- Các bản ghi lưu qua cơ chế này không tính vào tử số/mẫu số %NG chất lượng ở Andon board (chỉ `Ng` chủ động mới tính, giữ nguyên quy tắc US-18).
- Hệ quả cần lưu ý khi triển khai FR-20 (báo cáo tỷ lệ lỗi): số liệu chỉ phản ánh các lượt lỗi **đã được xác nhận lưu**, không phải toàn bộ lượt lỗi thực tế phát sinh tại trạm.
- Chi tiết AC: `Documents/BACKLOG-user-story.md` US-27.
...
- **Máy scan tem**: giao tiếp qua **chế độ keyboard-wedge (HID Keyboard giả lập bàn phím)** — **quyết định lại 17/08/2026** (trước đó bản SRS gốc yêu cầu đọc trực tiếp qua thư viện HID theo VendorID/ProductID, xem lý do đổi tại mục 6 quy tắc 7 và mục 8.1 dòng liên quan "trạm gộp nhiều công đoạn"). Vì mọi trạm triển khai đầu đều chỉ có 1 máy scan/1 công đoạn trên 1 PC (không có tình huống nhiều màn hình/nhiều công đoạn tranh nhau focus cửa sổ), keyboard-wedge là lựa chọn đơn giản, không cần thư viện HID riêng, chấp nhận được. **Đánh đổi đã biết và được chấp nhận**: input từ máy quét và input gõ tay/dán từ bàn phím thật là **không thể phân biệt được** ở tầng phần mềm (cùng là sự kiện bàn phím) — xem cơ chế bù trừ ("chế độ nhập tay/test") ngay dưới đây. Nếu về sau phát sinh trạm gộp nhiều công đoạn/nhiều màn hình trên 1 PC (mục 6 quy tắc 7), **phải** chuyển sang đọc trực tiếp HID theo VendorID/ProductID cho đúng tình huống đó.
- **Chế độ nhập tay/test (`EnableManualScanInput`)**: cấu hình cục bộ theo từng trạm (Options pattern, đọc từ `appsettings.json` tại trạm — cùng quy ước với `ArduinoTimeoutSeconds`/`NgModeTimeoutSeconds`, xem CLAUDE.md mục "Cấu hình"), **mặc định `false`** ở mọi trạm. Khi `true`: hiển thị rõ ràng 1 khu vực nhập tay mã tem trên màn hình trạm (khác với ô ẩn bắt input máy quét), kèm banner cảnh báo "ĐANG BẬT CHẾ ĐỘ NHẬP TAY" luôn hiển thị để không ai vô tình quên đang bật — dùng khi kiểm thử/demo mà chưa có máy quét vật lý. Do keyboard-wedge không phân biệt được nguồn input (xem trên), cờ này **không phải cơ chế chặn** gõ tay ở tầng kỹ thuật (máy quét vẫn gửi qua đúng kênh bàn phím dù cờ tắt) — mục đích chính là **giảm rủi ro vô tình lẫn dữ liệu giả vào production** bằng cách bắt buộc thao tác bật rõ ràng + cảnh báo trực quan, không phải bảo mật tuyệt đối. **Tuyệt đối không bật cờ này ở trạm production thật** — chỉ dùng ở môi trường dev/staging tách biệt dữ liệu sản xuất thật.
...
6. Lượt scan OK luôn được lưu lịch sử ngay lập tức. **Sửa lại 25/08/2026 (FR-27, đảo ngược 1 phần)**: lượt scan bị hệ thống **tự động từ chối** (trùng tem/chưa qua công đoạn trước/đang khóa rework/...) chỉ được lưu lịch sử khi Tổ trưởng/Admin/Manager đăng nhập xác nhận "Lưu" — bấm "Thoát" thì không có bản ghi nào, kể cả audit. Lượt Scan NG **chủ động** (US-18, người vận hành tự quét sản phẩm lỗi) không thuộc thay đổi này, vẫn luôn lưu ngay sau khi xác nhận.
7. Với trạm gộp nhiều công đoạn trên 1 PC (nhiều màn hình): bắt buộc mỗi màn hình chạy 1 tiến trình riêng, mỗi công đoạn có máy scan/Arduino riêng biệt. **Cập nhật 17/08/2026**: các trạm triển khai đầu đều chỉ 1 máy scan/1 công đoạn/1 PC nên tạm dùng keyboard-wedge (mục 5.1) — quy tắc "đọc trực tiếp HID theo VendorID/ProductID thay vì keyboard-wedge" ở đây chỉ **bắt buộc áp dụng lại** nếu/khi thực tế phát sinh trạm gộp nhiều công đoạn/nhiều màn hình (lúc đó keyboard-wedge không phụ thuộc được cửa sổ đang active để mỗi process chỉ lắng nghe đúng thiết bị của công đoạn mình, sẽ ghi nhầm dữ liệu giữa các công đoạn).
...
16. Công đoạn **"Đóng thùng"** (FR-24/FR-25/FR-26, bổ sung 20/08/2026) là 1 công đoạn bình thường trong danh mục/trình tự công đoạn — dùng lại nguyên vẹn FR-08 (chống trùng tem + kiểm tra công đoạn liền trước), **không có ngoại lệ ghi đè nào** (đúng quy tắc mặc định "từ chối cứng, không cho ghi đè" ở mục 8.1 — quyết định lại 20/08/2026, đảo ngược thiết kế nháp ban đầu). **Sửa lại 25/08/2026 (FR-27, đảo ngược lần nữa)**: khi tem trùng bị từ chối tại công đoạn này, việc lưu lịch sử dùng chung cơ chế Lưu/Thoát của FR-27 — Supervisor đăng nhập bấm "Lưu" mới tạo bản ghi (kèm danh tính người xác nhận), bấm "Thoát" thì không có bản ghi nào cả, kể cả audit. Không còn cơ chế audit riêng "luôn lưu Scan + xác nhận-đã-biết tùy chọn" như bản gốc 20/08/2026. Không áp dụng cơ chế này cho bất kỳ công đoạn nào khác ngoài phạm vi chung của FR-27.
...
21. Lượt scan bị **hệ thống tự động từ chối** (FR-27, bổ sung 25/08/2026) chỉ được lưu lịch sử khi Tổ trưởng/Admin/Manager đăng nhập xác nhận "Lưu" — bấm "Thoát"/Esc thì không lưu bất kỳ bản ghi nào, kể cả audit; áp dụng đồng nhất cho MỌI loại lỗi tự động (trùng tem, chưa qua công đoạn trước, đang khóa rework, kể cả tại công đoạn "Đóng thùng" — **thay thế hẳn** cơ chế audit riêng cũ của quy tắc 16), không phân biệt theo từng loại. Không áp dụng cho lượt Scan NG chủ động (US-18) — quy tắc 9 giữ nguyên.
...
| AC-31 | Tem đã được quét OK tại "Đóng thùng" trước đó, Operator quét lại đúng tem đó | Bị từ chối ngay theo đúng FR-08 (không ghi đè) — **sửa lại 25/08/2026 (FR-27)**: hiển thị thông báo Lưu/Thoát, CHƯA lưu gì; chỉ lưu lịch sử kèm danh tính người xác nhận khi Supervisor đăng nhập bấm "Lưu"; bấm "Thoát" thì không lưu gì cả (kể cả audit) — KHÔNG cộng thêm số lượng vào thùng hiện tại, KHÔNG tạo thêm bản ghi OK trong mọi trường hợp (mục 6 quy tắc 21, thay thế quy tắc 16 bản gốc) |
...
| AC-36 | Trạm được cấu hình cho công đoạn "Lắp ráp" (khác "Đóng thùng"), Operator scan trùng tem tại trạm đó | Luồng scan tiêu chuẩn (FR-07/FR-08) — không có bộ đếm/thùng, không tự in tem; tem trùng bị từ chối, **sửa lại 25/08/2026**: áp dụng đúng cơ chế Lưu/Thoát chung của FR-27 như mọi công đoạn khác (không còn là đặc thù riêng của "Đóng thùng" — FR-27 áp dụng đồng nhất toàn hệ thống) |
| AC-37 | Operator scan tem bị từ chối (trùng tem/chưa qua công đoạn trước/đang khóa rework), hệ thống hiển thị thông báo Lưu/Thoát | Bấm "Thoát"/Esc: đóng ngay, không lưu bất kỳ bản ghi nào. Bấm "Lưu": yêu cầu Tổ trưởng/Admin/Manager đăng nhập; đăng nhập đúng và có quyền `Scan.ConfirmReject` → lưu lịch sử kèm danh tính người xác nhận, thông báo "Lưu thành công" tự tắt sau 1-2 giây; đăng nhập sai/thiếu quyền/Hủy → không lưu gì (FR-27, mục 6 quy tắc 21) |
...
- [x] **Đã chốt 25/08/2026 (US-27, qua 2 vòng làm rõ với agent `ba`)** — Đảo ngược 1 phần quy tắc nền tảng "mọi lượt scan kể cả bị từ chối đều tự động lưu lịch sử" (FR-10/mục 6 quy tắc 6 gốc): lượt scan bị **hệ thống tự động từ chối** (trùng tem/chưa qua công đoạn trước/đang khóa rework/...) nay **chỉ lưu khi Tổ trưởng/Admin/Manager đăng nhập xác nhận "Lưu"**, bấm "Thoát" thì không lưu gì kể cả audit — chấp nhận hệ quả báo cáo tỷ lệ lỗi (FR-20) sau này chỉ thấy lỗi đã xác nhận, không phải toàn bộ lỗi thực tế phát sinh. Áp dụng đồng nhất mọi công đoạn, **thay thế hẳn** cơ chế audit riêng "tem trùng tại Đóng thùng" đã chốt 20/08/2026 (mục 6 quy tắc 16 cũ, nay là quy tắc 21). Ngoại lệ: Chế độ Scan NG chủ động (US-18, FR-18) giữ nguyên 100%, không thuộc phạm vi này. Xem FR-27, `Documents/backlog/US-27-xac-nhan-to-truong-truoc-khi-luu-scan-bi-tu-choi/README.md`.
